...
#blender blank.blend -b -P render_depth_map_of_full_mesh.py -- 0501 90
...
def setupBlender(angle_to_rotate):
	...
	light.location[0] = (0 + vmin[0] - vcenter_horizontal) / 2  #4.0762
	light.location[1] = (0 + vmax[1] - vcenter_vertical) / 2  #-151.59
	light.location[2] = vmax[2] * 8  #123.9
	...
	tree.links.new(rl.outputs["Depth"],fo.inputs["Image"])
...
os.rename( os.path.join(save_folder_path, "Image0001.exr") , os.path.join(save_folder_path, depth_map_filename + "_" + "{0:03d}".format( int(angle) ) + ".exr"  )  )
